...
describe('Server', () => {
  ...
  describe('CRUD', () => {
    ...
    it('Get url through /api/v1/urls/:id just fine', (done) => {
      app.get('/api/v1/urls/1').expect(200).end(done);
    });
    // Test deleting one data from DB
    it('Delete through /api/v1/urls/:id just fine', (done) => {
      app.delete('/api/v1/urls/1').expect(200).end(done);
    });
